...
@SpringBootTest
class BaseAccountServiceWithSynchronizedTest {

    @Autowired
    AccountServiceWithSynchronized accountService;

    @Autowired
    SynchronizedFacade synchronizedFacade;
...
//    @RepeatedTest(100)
    ...
    void sendWithMultiThreadTest() throws Exception {
        ...
        SoftAssertions sa = new SoftAssertions();
        sa.assertThat(sender.getBalance()).isNotEqualTo(initBalance - sendAmount * threadCount);
        sa.assertThat(receiver.getBalance()).isNotEqualTo(initBalance + sendAmount * threadCount);
        sa.assertAll();
    }
    ...
    //    @RepeatedTest(100)
    @Test
    void sendWithFacadeMultiThreadTest() throws Exception {
        // given
        int threadCount = 100;
        int sendAmount = 1000;
        ExecutorService executorService = Executors.newFixedThreadPool(32);
        ...
        for (int i = 0; i < threadCount; i++) {
            executorService.submit(() -> {
                try{
                    synchronizedFacade.send(sendRequest);
                } finally {
                    countDownLatch.countDown();
                }
            });
