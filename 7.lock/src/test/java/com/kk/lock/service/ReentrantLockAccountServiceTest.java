...
@SpringBootTest
class ReentrantLockAccountServiceTest {

    @Autowired
    ReentrantLockAccountService accountService;

    @Autowired
    ReentrantLockFacade reentrantLockFacade;
    ...
    void sendWithFacadeMultiThreadTest() throws Exception {
        ...
        for (int i = 0; i < threadCount; i++) {
            executorService.submit(() -> {
                try{
                    reentrantLockFacade.send(sendRequest);
                } finally {
                    countDownLatch.countDown();
                }
            });
