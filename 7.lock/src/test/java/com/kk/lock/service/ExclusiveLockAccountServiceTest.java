...
@SpringBootTest
class ExclusiveLockAccountServiceTest {

    @Autowired
    ExclusiveLockAccountService exclusiveLockAccountService;
    ...
    @Test
    void sendWithPessimisticLock() throws Exception {
        // given
        int threadCount = 100;
        int sendAmount = 1000;
        ...
        for (int i = 0; i < threadCount; i++) {
            executorService.submit(() -> {
                try{
                    exclusiveLockAccountService.sendWithPessimisticLock(sendRequest);
                } finally {
    ...
    @Test
    void sendWitNativeQueryLock() throws Exception {
        // given
        int threadCount = 100;
        int sendAmount = 1000;
        ExecutorService executorService = Executors.newFixedThreadPool(32);
        ...
        for (int i = 0; i < threadCount; i++) {
            executorService.submit(() -> {
                try{
                    exclusiveLockAccountService.sendWithNativeQueryLock(sendRequest);
                } finally {
                    countDownLatch.countDown();
                }
            });
