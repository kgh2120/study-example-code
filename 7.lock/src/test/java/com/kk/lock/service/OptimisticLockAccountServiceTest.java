...
@SpringBootTest
class OptimisticLockAccountServiceTest {

    @Autowired
    OptimisticLockAccountService optimisticLockAccountService;
    ...
    @Test
    void sendWithDataJpaOptimisticLockTest() throws Exception {
        // given
        int threadCount = 100;
        int sendAmount = 1000;
        ...
        for (int i = 0; i < threadCount; i++) {
            executorService.submit(() -> {
                try{
                    optimisticLockFacade.sendWithDataJpaOptimisticLock(sendRequest);
                } finally {
    ...
    @Test
    void sendWithEntityManagerOptimisticLockTest() throws Exception {
        // given
        int threadCount = 100;
        int sendAmount = 1000;
        ExecutorService executorService = Executors.newFixedThreadPool(32);
        ...
        for (int i = 0; i < threadCount; i++) {
            executorService.submit(() -> {
                try{
                    optimisticLockFacade.sendWithEntityManagerOptimisticLock(sendRequest);
                } finally {
                    countDownLatch.countDown();
                }
            });
